...
Object[][] tableData = dbHelper.getAllFilesTableData(user);

if (tableData != null && tableData.length > 0) {
                

    DefaultTableModel model = new DefaultTableModel(tableData, tableColumnNames);
    AllFilesTable.setModel(model);
    ...
    // Handle the case where the table data is empty
    JOptionPane.showMessageDialog(this, "No files available.", "Empty Table", JOptionPane.INFORMATION_MESSAGE);
}
    ...
    private void viewButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewButtonActionPerformed
        ...
        if (row >= 0 && row < AllFilesTable.getRowCount()) {
            String value = AllFilesTable.getModel().getValueAt(row, 2).toString();
            viewFileDetails = new ViewFileDetails(value,user);
            viewFileDetails.show();
        } 
        else {
        // Log or display an error message
            System.err.println("Error: Invalid row index or no row selected.");
        }
    ...
    private void downloadButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_downloadButtonActionPerformed
        ...
        int row = AllFilesTable.getSelectedRow();
        String value = AllFilesTable.getModel().getValueAt(row, 2).toString();
    if (AllFilesTable != null && AllFilesTable.getModel() != null && row >= 0) {
            try {
                // Rest of your code for handling the download
                fileOperations.downloadFile(value,user);
            } catch (GeneralSecurityException ex) {
                Logger.getLogger(AllFiles.class.getName()).log(Level.SEVERE, null, ex);
            }
    } else {
        // Log or display an error message
        System.err.println("Error: Invalid table or model, or no row selected.");
    }
